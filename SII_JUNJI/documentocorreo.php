...
$input2 = $_GET['idfact'];
$input["cliente_id"] = $_GET['idcli'];
$cliente = getClienteDetalle($input);
$dte = getDetalleDte($input2);
$correo1 =$cliente[1]["cliente_correo_contacto"];
$correo2 =$cliente[1]["cliente_correo_intercambio"];
$rutaxml =$dte[1]["dte_ruta"];
$namexml =$dte[1]["dte_archivo"];
$rutamasnombre =$rutaxml.$namexml.".xml";
$nombre_fichero_tmp = tempnam("/tmp", $namexml);
$contenidoxml = file_get_contents($rutamasnombre);
...
// PARAMETROS
$logo = array(
	0 => "includes/TCPDF-master/examples/images/_blank.png",
	1 => "../Nibiwi/Nibiwi.cl/images/logov7.png",
	2 => "includes/TCPDF-master/examples/images/junji.png",
	3 => "http://localhost/sistemas/inventario/privado/sitio2/junji.png",
	4 => "http://localhost/sistemas/inventario/privado/sitio2/junji_logo.png",
	5 => "../Nibiwi/Nibiwi.cl/images/logo.png"
	);
$logo = $logo[5];

$archivoXML = $rutamasnombre;
...
// TOTALES FINALES
			...
			<table border="0" width="100%">
				...
				// <br><br><br><br><br><br>
					...
					//$pdf->writeHTML($htmlnocedible, true, 0, true, 0);
										$pdf->Output($_SERVER['DOCUMENT_ROOT']."SII/tmp/F".$xml->SetDTE->DTE->Documento->Encabezado->IdDoc->Folio."T".$xml->SetDTE->Caratula->SubTotDTE->TpoDTE.".pdf", 'F');
										$rutapdf = "tmp/F".$xml->SetDTE->DTE->Documento->Encabezado->IdDoc->Folio."T".$xml->SetDTE->Caratula->SubTotDTE->TpoDTE.".pdf";
										$mail = new PHPMailer;

					//$mail->SMTPDebug = 3;                               // Enable verbose debug output
					$mail->isSMTP();                                      // Set mailer to use SMTP
					$mail->Host = 'mail.nibiwi.cl';  // Specify main and backup SMTP servers
					$mail->SMTPAuth = true;                               // Enable SMTP authentication
					$mail->Username = 'no-reply@nibiwi.cl';                 // SMTP username
					$mail->Password = 'brt@A^,L0-0K';                           // SMTP password
					$mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
					$mail->Port = 465;                                    // TCP port to connect to

					$mail->setFrom('no-reply@nibiwi.cl', 'TICMEGA S.P.A');
					// CORREO DE INTERCAMBIO
					$mail->addAddress($cliente[1]["cliente_correo_intercambio"],$cliente[1]["cliente_empresa"]);     // Add a recipient
					// CORREO CONTACTO
					$mail->AddCC($cliente[1]["cliente_correo_contacto"],$cliente[1]["cliente_empresa"]);

					$mail->addAttachment($rutamasnombre);         // Add attachments
					$mail->addAttachment($rutapdf);    // Optional name
					$mail->isHTML(true);                                  // Set email format to HTML

					$mail->Subject = 'Documentos pdf,xml adjuntos de la factura Numero:'.$dte[1]["dte_folio"]." | fecha :".$dte[1]["dte_fecha"];
					$mail->Body    = 'Estimado '.$cliente[1]["cliente_empresa"].' Rut: '.number_format($cliente[1]["cliente_rut"],0,".",".").'-'.$cliente[1]["cliente_dv"].' mediante este correo adjuntamos los archivos .pdf y .xml solicitados en nuestro sitio.';
