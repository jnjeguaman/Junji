...
$rutaXML = "../sistemas/archivos/SII/".$detalleDTE[1]["dte_ruta"].$detalleDTE[1]["dte_archivo"].".xml";
$contenidoXML = file_get_contents($rutaXML);
$arrayXML = simplexml_load_string($contenidoXML);

$datosXML = [
"RazonSocial" => (string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Receptor->RznSocRecep,
"Rut" => explode("-",(string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Receptor->RUTRecep),
"Direccion" => (string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Receptor->DirRecep,
"Comuna" => (string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Receptor->CmnaRecep,
"Ciudad" => (string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Receptor->CiudadRecep,
"Giro" => (string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Receptor->GiroRecep,
"actividadEconomica" => (string)$arrayXML->SetDTE->DTE->Documento->Encabezado->Emisor->Acteco,
];
$datosReceptor = array();
if($detalleDTE[1]["dte_dcto_id"] == 52)
{
	...
	$datosReceptor = [
		"RazonSocial" => $datosXML["RazonSocial"],
		"Rut" => $datosXML["Rut"][0],
		"Dv" => $datosXML["Rut"][1],
		"Direccion" => $datosXML["Direccion"],
		"Comuna" => $datosXML["Comuna"],
		"Ciudad" => $datosXML["Ciudad"],
		"Giro" => $datosXML["Giro"],
		"actividadEconomica" => $datosXML["actividadEconomica"],
	];
}else{
