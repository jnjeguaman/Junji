...
/**
 * Clase para trabajar con las Notas de Crédito Electrónicas
...
class NotaCredito
{
	...
	function __construct($input)
	{
		...
		$esValido = $objFirma->validaUsuario($this->siiDatos["emisor_region"],61,$this->siiDatos["emisor_rut"]);
	...
	**/
	public function actualizaCliente($input)
	{
		try {
			...
			$null = NULL;
			$query = "UPDATE sii_cliente SET cliente_empresa = ?, cliente_rut = ?, cliente_dv = ?, cliente_direccion = ?, cliente_provincia_id = ?, cliente_comuna_id = ?, cliente_giro = ? WHERE cliente_id = ?";
	...
	**/
	private function GeneraDocumento()
	{
		...
		$Documento->generate([
			...
			"CiudadRecep" => substr(utf8_encode($this->siiDatos["CiudadRecep2"]),0,20),
			],
			"Totales" => [
			"MntNeto" => ($this->siiDatos["CodRef"] == 2 || $this->siiDatos["TpoDocRef"] == 34) ? false : $neto,
			"MntExe" => ($this->siiDatos["CodRef"] == 2) ? 0 : $this->siiDatos["cotizacion_exento"],
			"TasaIVA" => ($this->siiDatos["TpoDocRef"] == 34) ? false : 19,
			"IVA" => ($this->siiDatos["CodRef"] == 2 || $this->siiDatos["TpoDocRef"] == 34) ? false : round($neto * 0.19),
			"MntTotal" => ($this->siiDatos["CodRef"] == 2) ? 0 : ( round($neto * 0.19) + $neto + $this->siiDatos["cotizacion_exento"]),
			],
				],//Encabezado
				"Detalle" => $Detalle->getElementsByTagName("DATA")->item(0),
				"DscRcgGlobal" => ($this->siiDatos["DscRcgGlobal"] <> "on" || $this->siiDatos["CodRef"] == 2) ? false : $DscRcgGlobal->getElementsByTagName("DscRcgGlobal")->item(0),
				// "Referencia" => $Referencia->getElementsByTagName("DATA")->item(0),
				"Referencia" => [
				// [
				// "NroLinRef" => 1,
				// "TpoDocRef" => "SET",
				...
				"CodRef" => ($this->siiDatos["CodRef"] == 4) ? 1 : $this->siiDatos["CodRef"],
	...
	**/
	private function EnvioDTE()
	{
		...
		// $doc = new XML();
		// $doc->loadXML(file_get_contents("temp".$this->referencia.".xml"));
		...
		if($Schema <> ""){return array("Respuesta" => false,"Mensaje" => "Se ha producido un error en el Schema del documento generado. ".$Schema);}
		if($Firma <> ""){return array("Respuesta" => false,"Mensaje" => "Ha ocurrido un error al firmar el documento con su firma digital. ".$Firma);}
	...
	private function GeneraDetalle()
	{
		...
		if($this->siiDatos["CodRef"] <> 2)
		{
			for ($i=0; $i < $totalElementos; $i++) {
				...
				{
					if($this->siiDatos["var12"][$i] == "on" || $this->siiDatos["CodRef"] == 1)
					{
						$Detalle->generate([
							"Detalle" => [
							"NroLinDet" => $cont,
							"IndExe" => ($this->siiDatos["var10"][$i] == 1) ? 1 : false,
			...
			$Detalle->generate([
				"Detalle" => [
				"NroLinDet" => $cont,
				"NmbItem" => self::decode(utf8_encode($this->siiDatos["RazonRef"])),
				"MontoItem" => 0,
				],
				]);
		}
	...
	**/
	private function GeneraTED()
	{
		...
		$TED->generate([
			...
			'MNT' => ($this->siiDatos["CodRef"] == 2) ? 0 : ( round($neto * 0.19) + $neto + $this->siiDatos["cotizacion_exento"]),
			'IT1' => ($this->siiDatos["CodRef"] == 2) ? substr($this->siiDatos["RazonRef"],0,40) : substr($this->siiDatos["var5"][0],0,40),
			'CAF' => $objCAF->getCAF2(),
		...
		// $DD = str_replace('"','\"',$DD);
		// $fp = fopen("../Documentos/".$this->tipo_dcto."/dd_".$this->referencia.".xml","w+");
