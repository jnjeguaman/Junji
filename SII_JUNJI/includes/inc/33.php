...
/**
 * Clase para trabajar con la Factura Electronica
...
class FacturaElectronica
{
	...
	function __construct($input)
	{
		...
		$objCAF = new CAF($input["tipo_dcto"],$input["emisor_region"]);
		...
		$esValido = $objFirma->validaUsuario($this->siiDatos["emisor_region"],33,$this->siiDatos["emisor_rut"]);
	...
	**/
	private function GeneraDocumento()
	{
		...
		$Documento->generate([
			...
			"Totales" => [
			"MntNeto" => $this->siiDatos["neto"],
			"MntExe" => $this->siiDatos["exento"],
			"TasaIVA" => 19,
			"IVA" => $this->siiDatos["iva"],
			"MntTotal" => $this->siiDatos["total"],
			],
				],//Encabezado
			...
			if($this->token === "Service Temporarily Unavailable")
			{
				...
				try {
					...
					{	
						...
						if($res->STATUS == 0)
						{
							$objDTE->nuevoDTE($this->referencia,$this->tipo_dcto,$this->folio,$res,$cotizacion[1]["cotizacion_cliente_id"],$this->cotizacion,$this->siiDatos["emisor_region"]);
							$objCotizacion->actualizarEstado($this->cotizacion,33);
