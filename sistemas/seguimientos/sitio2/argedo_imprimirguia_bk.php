...
<table border="1" width="100%" align="center" cellpadding="5">
  ...
  while ($row31 = mysql_fetch_array($res31)) {
    ...
    <tr>
      ...
      if(utf8_encode($row31["reci_tipodoc"]) == "LICENCIA MÉDICA")
      {
        $html.=utf8_encode($materia).'<br>INICIO REPOSO : '.date("d-m-Y",strtotime($row32["lice_ini_reposo"]));
      }else{
        $html.=utf8_encode($materia);
      }
      $html.='</td>';
      $html.='<td style="font-size:0.6em;">'.utf8_encode($row31["reci_destinatario"]).'</td>
