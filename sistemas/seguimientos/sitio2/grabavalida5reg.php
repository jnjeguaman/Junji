...
//         $sql21="select max(eta_folioguia2) as foliomio from dpp_etapas where eta_region='$regionsession' ";
         $sql21="select max(eta_folioguia3) as foliomio from dpp_etapas where eta_region='$regionsession' and year(eta_fecha_ing)='$annomia'";
//         echo $sql21;
//         exit();
...
//   echo $var1."----".$uno;
     ...
     if ($var1<>"" and $uno==1) {
         // $sql1="update dpp_etapas set eta_estado=6, eta_usu_jefatura3='$usuario', eta_fecha_jefatura3 ='$fechamia', eta_folioguia2='$foliomio',eta_destinatario3='$destinatario', eta_fechaguia3='$fechamia'  where eta_id=$var1 ";
        $sql1="update dpp_etapas set eta_estado=5, eta_destinatario3='$destinatario', eta_folioguia3 = '$foliomio', eta_fechaguia3 = '".$full_date."'  where eta_id=$var1 ";
         // echo $sql1;
         mysql_query($sql1);
     }
...
if($enviarEmail)
{
    ...
    $sql31="select * from dpp_etapas where eta_region='$regionsession' and year(eta_fechaguia3)='$annomia' and eta_folioguia3=$foliomio order by eta_folio desc";
    ...
    $msg.='<br><br>Atentamente, Contabilidad<br>'.$row32["nombre"];
...
$sql4 = "SELECT * FROM usuarios WHERE (atributo1 = 31) AND sistema = 1 AND estado = 'A' AND region = ".$regionsession;
...
echo "<script>location.href='valida5reg.php?llave=1';</script>";
