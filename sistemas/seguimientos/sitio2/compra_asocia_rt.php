...
$sql = "SELECT * FROM bode_orcom a INNER JOIN bode_ingreso b on b.ing_oc_id = a.oc_id WHERE a.oc_id2 = '".$oc."' AND b.ing_guianumerotc <> 0 AND b.ing_region = ".$region." AND b.ing_aprobado <> ''";
$res = mysql_query($sql,$dbh6);
$contador = 0;
?>
<form action="compra_asocia_rt.php" method="POST">
<table border="0" width="100%" style="border:1px solid #000;border-radius: 10px;border-collapse: collapse;text-align: center;">
	<tr>
		...
		<td style="font-size: 0.8em;border-bottom: 1px solid #000;border-right: 1px solid #000;background-color: #bdd4ff;">N&deg; RECEPCION TECNICA</td>
		<td style="font-size: 0.8em;border-bottom: 1px solid #000;border-right: 1px solid #000;background-color: #bdd4ff;">RECEPCTION TECNICA</td>
...
<?php endif ?>
	...
	<td style="font-size: 0.8em;border-bottom: 1px solid #000;border-right: 1px solid #000;"><?php echo $row["ing_guianumerotc"] ?></td>
	<td style="font-size: 0.8em;border-bottom: 1px solid #000;border-right: 1px solid #000;"><a href="../../inventario/privado/sitio2/bode_tca.php?numguia=<?php echo $row["ing_guianumerotc"] ?>" target="_blank" style="text-decoration: none;">VER</a></td>
...
if($enviado == 1)
{
	for ($i=0; $i < $totalElementos ; $i++) { 
		if($var1[$i] <> 0)
		{
			$sql2 = "INSERT INTO compra_doc_inedis VALUES ('','".$id."','".$regionSession."','".$nom_user."','".$fechamia."','".$horamia."','RT','".$var1[$i]."',1)";
