...
$params = array('trace' => 1, "porCode" => $codigo, 'login' => $mercadoPublico[0], 'password' => $mercadoPublico[1]);
...
//  $contenido2 = utf8_encode ($contenido2);
  ...
  $direccion= $xmlObject->OrdersList->Order->OrderHeader->OrderNumber->BuyerOrderNumber;
 // $direccion= $xmlObject->OrdersList->Order->OrderHeader->OrderReferences->QuoteReference->RefNum."|";
 $obs=$xmlObject->OrdersList->Order->OrderHeader->OrderReferences->OtherOrderReferences->ReferenceCoded->ReferenceDescription;
...
$dig= $rutparte[1]; //porción2
  ...
  $rut = str_replace(".","",$rut);
   ...
   $descuento=$xmlObject->OrdersList->Order->OrderSummary->AllowOrChargeSummary->TotalAllowOrCharge->SummaryAllowOrCharge->MonetaryAmount;
   if ($descuento<0) {
       $descuento=$descuento*-1;
   }
   $nombreoc=$xmlObject->OrdersList->Order->OrderHeader->OrderReferences->QuoteReference->RefNum;
/*
//  echo $rut."|";
