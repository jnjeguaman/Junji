...
$sql="SELECT * FROM dpp_etapas WHERE eta_estado=1 AND eta_folioguia<>0 AND eta_region='$regionSession' AND (eta_rut='$rut' AND eta_dig='$dig') AND eta_fecha_recepcion >= '2017-02-01' AND eta_tipo_doc2 = 'n' ORDER BY eta_folio DESC";
//$sql = "SELECT * FROM bode_orcom a INNER JOIN bode_ingreso b on b.ing_oc_id = a.oc_id WHERE a.oc_id2 = '".$oc."'";
//echo $sql;
$res = mysql_query($sql,$dbh);
$contador = 0;
?>
<form action="compra_asocia_nc.php" method="POST">
...
if($enviado == 1)
{
	for ($i=0; $i < $totalElementos ; $i++) { 
		if($var1[$i] <> 0)
		{
			$sql2 = "INSERT INTO dpp_etapas_nota VALUES ('','".$id."','".$var1[$i]."','".$var2[$i]."','".$var3[$i]."','NC','".$nom_user."','".$fechamia."','".$regionSession."',1)";
