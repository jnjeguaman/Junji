...
<?
  $ocid=$row3["oc_id"];
  $sql44="Select * from compra_oceta x, dpp_etapas y where x.oceta_oc_id =$ocid and x.oceta_eta_id=y.eta_id";
//  echo $sql44."<br>";
  $consulta44=mysql_query($sql44);
  while ($reg44=mysql_fetch_array($consulta44)) {
  $octipo2=$reg44["cat_nombre"];

 ?>
    <tr>
     <td class="Estilo3c" colspan=3></td>
     <td class="Estilo3c">FACTURA </td>
     <td class="Estilo2"><? echo $reg44["eta_monto"]  ?></td>
     <td class="Estilo3c" ><? echo substr($reg44["eta_fecha_recepcion"],8,2)."-".substr($reg44["eta_fecha_recepcion"],5,2)."-".substr($reg44["eta_fecha_recepcion"],0,4)   ?></td>
     <td class="Estilo2"><? echo $reg44["eta_item"]  ?>.<? echo $reg44["eta_item2"]  ?>.<? echo $reg44["eta_asig"]  ?></td>
     <td class="Estilo3c" ><? echo substr($reg44["eta_fdevengo"],8,2)."-".substr($reg44["eta_fdevengo"],5,2)."-".substr($reg44["eta_fdevengo"],0,4)   ?></td>
    </tr>
