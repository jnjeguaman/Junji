...
$sql2b = "Select * from argedo_documentos where docs_id in ($argedo2)";
//            echo $sql2;
 $res2b = mysql_query($sql2b);
while($row2b = mysql_fetch_array($res2b)){
    $docsid=$row2b["docs_id"];
    $sql="INSERT INTO cometido_ree_res (reeres_ree_id, reeres_docs_id)
                            VALUES ($maximo,$docsid)";
    //echo $sql."<br>";
    //exit();
    mysql_query($sql);
    
    $sql="update cometido_funcionario set come_swree=1, come_certificado='Si', come_fechacertificado='$fecha1' where come_docs_id=$docsid";
    //echo $sql."<br>";
    //exit();
    mysql_query($sql);
}
...
if ($origen==1) {
  echo "<script>location.href='compra_orden3b.php?llave=1';</script>";
}
