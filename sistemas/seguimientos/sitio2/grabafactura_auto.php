...
//  echo $sql1;
  ...
  $ultimoETAID = mysql_insert_id();
  $sql6 = "UPDATE dpp_facturas_antecedente SET ant_eta_id = '".$ultimoETAID."' WHERE ant_region = '".$regionsession."' AND ant_usuario = '".$_SESSION["nom_user"]."' AND ant_eta_id IS NULL AND ant_estado = 1";
  mysql_query($sql6);
...
mkdir("../../archivos/docfac/doc".$annomia."/factura",0777,true);
    $destino =  "../../archivos/docfac/".$archivo1;

    /* ESTAMPA  */
          $temporal = "TEMP_".$maximo.".PDF"; // Archivo Temporal
          $destinoTemporal = "../../archivos/".$temporal; // Carpeta Temporal

		  
          // Copiamos en la carpeta Temporal
          if(copy($_FILES["archivo1"]["tmp_name"], $destinoTemporal))
          {
            // SI SE COPIO PROCEDEREMOS A INSERTAR EL TIMBRE

              $cmd = "gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=$destino ".$postScript[$regionsession]." ".$destinoTemporal;
            putenv("PATH=/usr/local/bin:/usr/bin:/bin");
            exec($cmd);
            $sql2="update dpp_facturas set fac_archivo='$archivo1' where fac_eta_id=$maximo ";
            mysql_query($sql2);
            unlink($destinoTemporal);
          }
		  else
		  {
			  $content = 'falla if copy($_FILES["archivo1"]["tmp_name"], $destinoTemporal) '.var_export($_FILES['archivo1']['error'], true);
				$fp = fopen("../../archivos/docfac/".$maximo.".txt" ,"wb");
				fwrite($fp,$content);
				fclose($fp);
		  }
...
//            echo "Error en archivo no copiado ";
  ...
  else
  {
		...
		$content = '($archivo1 != "") dió falso; '.$_FILES['archivo1']['error'] ;
		$fp = fopen("../../archivos/docfac/".$maximo.".txt " ,"wb");
		fwrite($fp,$content);
		fclose($fp);
  }
