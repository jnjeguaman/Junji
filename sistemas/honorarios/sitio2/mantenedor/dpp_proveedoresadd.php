...
if (@$_POST["a_add"] <> "") {
  $dpp_proveedores->CurrentAction = $_POST["a_add"]; // Get form action
  LoadFormValues(); // Load form values
} else { // Not post back
  if ($bCopy) {
    $dpp_proveedores->CurrentAction = "C"; // Copy Record
  } else {
    $dpp_proveedores->CurrentAction = "I"; // Display Blank Record
...
switch ($dpp_proveedores->CurrentAction) {
  ...
  case "C": // Copy an existing record
   if (!LoadRow()) { // Load record based on key
      ...
      Page_Terminate($dpp_proveedores->getReturnUrl()); // Clean up and return
    }
		break;
  case "A": // ' Add new record
		$dpp_proveedores->SendEmail = TRUE; // Send email on add success
    if (AddRow()) { // Add successful
      $_SESSION[EW_SESSION_MESSAGE] = "Nuevo registro agregado satisfactoriamente"; // Set up success message
      Page_Terminate($dpp_proveedores->KeyUrl($dpp_proveedores->getReturnUrl())); // Clean up and return
    } else {
...
$dpp_proveedores->RowType = EW_ROWTYPE_ADD;  // Render add type
RenderRow();
?>
...
<p><span class="phpmaker">Agregar TABLA: dpp proveedores<br><br><a href="<?php echo $dpp_proveedores->getReturnUrl() ?>">Volver atras</a></span></p>
<?php
...
?>
<form name="fdpp_proveedoresadd" id="fdpp_proveedoresadd" action="dpp_proveedoresadd.php" method="post" onSubmit="return ew_ValidateForm(this);">
<p>
...
function LoadDefaultValues() {
	...
	$dpp_proveedores->provee_rut->CurrentValue = 0;
}
?>
<?php
...
function AddRow() {
	...
	$bCheckKey = TRUE;
	$sFilter = $dpp_proveedores->SqlKeyFilter();
	if (trim(strval($dpp_proveedores->provee_id->CurrentValue)) == "") {
		$bCheckKey = FALSE;
	} else {
		$sFilter = str_replace("@provee_id@", ew_AdjustSql($dpp_proveedores->provee_id->CurrentValue), $sFilter); // Replace key value
	}
	...
	if ($bCheckKey) {
		$rsChk = $dpp_proveedores->LoadRs($sFilter);
		if ($rsChk && !$rsChk->EOF) {
	...
	$bInsertRow = $dpp_proveedores->Row_Inserting($rsnew);
	if ($bInsertRow) {
		$conn->raiseErrorFn = 'ew_ErrorFn';
		$AddRow = $conn->Execute($dpp_proveedores->InsertSQL($rsnew));
		$conn->raiseErrorFn = '';
	} else {
	...
	if ($AddRow) {
		$dpp_proveedores->provee_id->setDbValue($conn->Insert_ID());
		$rsnew['provee_id'] =& $dpp_proveedores->provee_id->DbValue;

		// Call Row Inserted event
		$dpp_proveedores->Row_Inserted($rsnew);
	}
	return $AddRow;
}
?>
