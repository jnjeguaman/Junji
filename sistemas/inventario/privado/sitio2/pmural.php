...
<body>
	...
	<div  style="width:800px; background-color:#E0F8E0; position:absolute; top:120px; left:00px;" id="div1">
		...
		<br>
		<form action="pmural.php" method="POST">
		...
		if($atributo === 23)
		{
			$sql2 = "SELECT * FROM acti_inventario WHERE  inv_responsable LIKE '%".$responsable."%' AND inv_estado2 = 1 and inv_region = ".$_SESSION["region"]." ORDER by inv_id desc limit 400";
		}else{
			...
			$sql2 = "SELECT * FROM acti_inventario WHERE inv_region = ".$_SESSION["region"]." $where AND inv_estado2 = 1 ORDER by inv_id desc LIMIT 400";
		}
		...
		<table border="0" width="100%">
			...
			while ($row2 = mysql_fetch_array($res2)) {
				...
				<tr class="<?php echo $estilo2 ?> trh">
					...
					<td ><a href="pmural.php?responsable=<?php echo $row2["inv_responsable"] ?>"><?php echo $row2["inv_responsable"] ?></a></td>
...
<div  style="width:540px; background-color:#E0F8E0; position:absolute; top:120px; left:805px;" id="div2">
	<?php 
	$sql = "SELECT COUNT(inv_codigo) AS Total,inv_estadocosto, inv_codigo, inv_bien, inv_costo, inv_obs FROM acti_inventario WHERE inv_region = '".$_SESSION["region"]."' $where AND inv_visible = 1 AND (inv_baja = '' or inv_baja IS NULL) GROUP BY inv_bien";
	$sql = mysql_query($sql);
	...
	<?php else: ?> 
		...
		<br>
		<form action="generar/index.php" method="POST" target="_blank">
			<table border="1" width="100%">
				<thead>
				...
				<tbody>
					<?php $cont = 1;  while ($row = mysql_fetch_array($sql)) { 
						...
						<tr>
							<td colspan="3" align="center"><button type="submit" name="tipoDCTO" value="certificado">GENERAR CERTIFICADO</button></td>
						</tr>
			...
			<?php if ($atributo != 23): ?>
				...
				<table border="0" width="100%" cellpadding="0" cellspacing="0">
					<form action="generar/index.php" method="POST" target="_blank" onSubmit="return planilla()">
						<tr>
						...
						<tr>
							<td class="Estilo1mc" colspan="4"><button type="submit" name="tipoDCTO" value="pmural">GENERAR PLANILLA MURAL</button></td>
						</tr>
