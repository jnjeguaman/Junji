...
<div id="seccion1" style="background-color:#E0F8E0;" >
	<table border=0 width="100%">
        ...
        <table border="0" width="100%">
          <tr>
            <td><input type="radio" name="swtipo" value="0" <? if ($swtipo=="0") { echo "checked"; } ?>> <label>Todos</label></td>
            <td><input type="radio" name="swtipo" value="1" <? if ($swtipo=="1") { echo "checked"; } ?>> <label>Negro Mayores a <? echo $plabode1 ?> Dias</label></td>
            <td><input type="radio" name="swtipo" value="2" <? if ($swtipo=="2") { echo "checked"; } ?>><font color="#088A08"> Verde Entre  <? echo $plabode1 ?> a <? echo $plabode2 ?> Dias</font></td>
            <td><input type="radio" name="swtipo" value="3" <? if ($swtipo=="3") { echo "checked"; } ?>><font color="#2E2EFE"> Azul Entre <? echo $plabode2 ?> a <? echo $plabode3 ?> Dias </font></td>
            <td><input type="radio" name="swtipo" value="4" <? if ($swtipo=="4") { echo "checked"; } ?>><font color="#DEAF00"> Marrón Entre 0 a <? echo $plabode3 ?> Dias </font></td>
            <td><input type="radio" name="swtipo" value="5" <? if ($swtipo=="5") { echo "checked"; } ?>><font color="#FE2E2E"> Fuera de plazo</font></td>
          </tr>
        </table>
		...
		{
			$sql2 = "SELECT * FROM bode_detoc x, bode_orcom y  WHERE (x.doc_region = 16  OR x.doc_region = 13)and x.doc_oc_id=y.oc_id  and (x.doc_estado='' or  x.doc_estado='CO') and x.doc_estadocierre=0 $wherenivel40 $whereoc AND y.oc_estado = 1 AND y.oc_tipo = 0 AND oc_usu <> 'INEDIS'/*AND x.doc_cantidad = x.doc_recibidos*/ ORDER by doc_id desc limit 0,100";
			//echo $sql2;

		}else{
			$sql2 = "SELECT * FROM bode_detoc x, bode_orcom y  WHERE x.doc_region = ".$_SESSION["region"]." and x.doc_oc_id=y.oc_id  and (x.doc_estado='' or  x.doc_estado='CO') and x.doc_estadocierre=0 $wherenivel40 $whereoc  AND (y.oc_tipo = 0 OR y.oc_tipo = 1) AND oc_usu <> 'INEDIS ORDER by doc_id desc limit 0,100";
		}
...
//        echo "-->".$unob;
                                          			...
                                          			<?php else: ?>
                                          				<a href="bode_inv_indexoc2.php?ori=4&oc_id=<? echo $row2["doc_oc_id"] ?>&doc_id=<? echo $row2["doc_id"] ?><? ($row2b["ding_ing_id"] <> 0) ? "&ing_id=".$row2b["ding_ing_id"] : "" ?>" class="link"><i class="fa fa-plus"></i> <? echo $ojo ?></a>
                                          			<?php endif ?>
                                          			<?
