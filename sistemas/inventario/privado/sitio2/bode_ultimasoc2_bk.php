...
<div id="seccion1" style="background-color:#E0F8E0;" >
	<table border=0 width="100%">
		...
		<form method="get" action="bode_inv_indexoc2.php" name="filtrorecepcion">
			...
			<tr>
				<td  class="Estilo2" colspan="10">
					...
					<input type="radio" name="swtipo" value="4" <? if ($swtipo=="4") { echo "checked"; } ?>><font color="#FE2E2E"> Rojo Menores a <? echo $plabode3 ?> Dias </font>
				</td>
			</tr>
		...
		{
			$sql2 = "SELECT * FROM bode_detoc x, bode_orcom y  WHERE (x.doc_region = 16  OR x.doc_region = 13)and x.doc_oc_id=y.oc_id  and (x.doc_estado='' or  x.doc_estado='CO') and (x.doc_estadocierre=0 OR x.doc_estadocierre=1) $wherenivel40 $whereoc  ORDER by doc_id desc limit 0,200";
                                          ...
                                          if ($swcolor==$swtipo or $swtipo=='' or $swtipo=='0') {
                                          	...
                                          	<tr>
                                          		...
                                          		<td  class="<? echo $estilo2 ?>">
                                          			...
                                          			if ($row2["doc_recibidos"]< $row2["doc_cantidad"] and $nivel<>40)  {
                                          				?>
...
//        echo "-->".$unob;
                                          			...
                                          			?>
                                          			<?php if($usuario_id === 365 || $usuario_id == 47 || $usuario_id === 367 || $usuario_id=== 369 || $usuario_id === 366): ?>
                                          				<a href="bode_inv_indexoc2.php?ori=4&oc_id=<? echo $row2["doc_oc_id"] ?>&doc_id=<? echo $row2["doc_id"] ?>&total=<?php echo $row2["compra_cantidad"] ?>" class="link"><i class="fa fa-plus"></i> <? echo $ojo ?></a>
                                          			<?php endif ?>
                                          			<?
                                          		...
                                          		<td  class="<? echo $estilo2 ?>">
                                          			<?
                                          			echo $row2["doc_final"];
                                          			if ($nivel<>40 AND $usuario_id === 47 || $usuario_id === 367 || $usuario_id=== 369 || $usuario_id === 368 || $usuario_id === 366) {
                                          				?>
