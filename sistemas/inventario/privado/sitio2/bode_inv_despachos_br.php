...
// BUSCAMOS EL DETALLE DEL PRODUCTO
  ...
  // $sql2 = "SELECT * FROM bode_detoc a inner join bode_detingreso b on a.doc_id = b.ding_prod_id where b.ding_ubicacion = 'CD - DIRNAC' AND a.doc_numerooc = '".$row["doc_numerooc"]."' AND a.doc_id_mercado_publico = '".$row["doc_id_mercado_publico"]."' LIMIT 1";
     $sql2 = "SELECT * FROM bode_detoc a INNER JOIN bode_detingreso b ON b.ding_prod_id = a.doc_id WHERE a.doc_id_mercado_publico = '".$row["doc_id_mercado_publico"]."' AND b.ding_ubicacion =  'CD - DIRNAC' LIMIT 1";
