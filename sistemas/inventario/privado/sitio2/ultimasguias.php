								<div id="seccion1" style="background-color:#E0F8E0;" >
									<table border=0 width="100%">
										...
										<?
		                                $sql2 = "SELECT * FROM bode_orcom y WHERE y.oc_region = ".$_SESSION["region"]." and oc_id2='111111'  ORDER by oc_id desc limit 0,200";
...
//										$sql2 = "SELECT * FROM acti_compra_temporal WHERE (estado = 0 or estado=1) ORDER by compra_id desc limit 0,20";
										...
										while ($row2 = mysql_fetch_array($res2)) {
											...
											<tr>
												...
												<td  class="<? echo $estilo2 ?>"><? echo $row2["oc_fecha_recep"] ?></td>
            									<td  class="<? echo $estilo2 ?>"><? echo $row2["doc_cantidad"] ?></td>
												<td  class="<? echo $estilo2 ?>"><a href="inv_indexoc3.php?ori=3&id=<? echo $row2["oc_id"] ?>&id2=<? echo  $row2["doc_id"] ?>&total=<?php echo $row2["compra_cantidad"] ?>" class="link"><i class="fa fa-plus"></i></a></td>
												<td  class="<? echo $estilo2 ?>"><a href="inv_indexoc2.php?ori=4&oc_id=<? echo $row2["oc_id"] ?>&doc_id=<? echo $row2["doc_id"] ?>&total=<?php echo $row2["compra_cantidad"] ?>" class="link"><i class="fa fa-eye"></i></a></td>
