...
//            echo $sql2;
		...
		$v_oc_sp_id = $row2["oc_sp_id"];

		//     echo $v_oc_region;

		/* BUSCAMOS LA REGION DE DESTINO SEGUN TIPO DE GUIA
		 1 : BODEGA
		 2 : OFICINA
		 3 : JARDIN
		 5 : CONSUMO <-- SE OMITE DTE
		 6 : TRASLADO
		 */

		 if($v_oc_tipoguia == 1)
		 {
		 	$v_oc_region_destino = $row2["oc_region"];
		 }

		 if($v_oc_tipoguia == 2)
		 {
		 	$v_oc_region_destino = $row2["oc_region2"];
		 }

		 if($v_oc_tipoguia == 3)
		 {
		 	$sql5 = "SELECT jardin_region as regionDestino FROM jardines WHERE jardin_codigo = '".$v_oc_region."' and jardin_estado = 1";
		 	$res5 = mysql_query($sql5,$dbh);
		 	$row5 = mysql_fetch_array($res5);
		 	$v_oc_region_destino = $row5["regionDestino"];
		 }
		 ...
		 if($v_oc_sp_id <> "")
		 {
		 	$sql7 = "SELECT * FROM bode_solicitud WHERE sp_id = ".$v_oc_sp_id;
		 	$res7 = mysql_query($sql7,$dbh);
		 ...
		 <form name="f1" method="POST" action="bode_inv_indexoc3.php?ori=<?php echo $ori ?>&id=<?php echo $id?>" onSubmit="blockUI();">
		 	<table border="0"  width="100%">
		 		...
		 		<tr>
		 			...
		 			<td class="Estilo2">  <input type="text" name="numoc" size="20" value="<? if(isset($numoc)){echo $numoc;} ?>"> </td>
		 		</tr>

		 		<tr>
		 			<td class="Estilo1c">Especificacion </td>
		 			<td class="Estilo2">  <input type="text" name="especificacion" size="20" value="<? if(isset($especificacion)){echo $especificacion;} ?>"> </td>
		 		</tr>
		 		...
		 		<tr>
					...
					<?php if ($_SESSION["region"] == 16): ?>
		 					...
		 					<?php if ($_SESSION["atributo1"] == 38 || $_SESSION["atributo1"] == 35 || $_SESSION["atributo1"] == 50): ?>
		 					...
		 					<?php else: ?>
		 						<input type="radio" name="tipo" value="Existencia" checked onClick="this.form.submit()">EXISTENCIA
		 					<?php endif ?>
...
<!--
												...
												-->
												<?
												$limite = 50; //CANTIDAD MAXIMA DE RESULTADOS POR PAGINA
												if ($numoc<>'') {
												...
												while ($row3 = mysql_fetch_array($res4)) {
													...
													<tr>
														...
														<td class="Estilo1">
															<?
															if ($row3["doc_factor"] == 0) {
																$precio1=0;
																$precio2=0;
															}else{
																$precio1=number_format($row3["doc_conversion"] / $row3["doc_factor"],0,',','.');
																$precio2=$row3["doc_conversion"] / $row3["doc_factor"];
															} 


															?>
															<input type="text" name="var5b[<? echo $i ?>]" class="Estilo2" size="7"  disabled value="$<? echo $precio1; ?>" >
															<input type="hidden" name="var5[<? echo $i ?>]" class="Estilo2" size="7"   value="<? echo $precio2; ?>" >
									...
									<form action="bode_inv_despachos_gr_multi.php" method="POST">
										...
										<table border="0" width="100%">
											...
											<?
											// TOTAL DE INVENTARIABLES SEGUN CLASIFICACION DEL PRODUCTO Y GUIA ASOCIADA;
											...
											while ($row3 = mysql_fetch_array($res3)) {
												...
												<tr>
													...
													<td class="Estilo1" colspan=1><input type="text" name="var3b[<? echo $i ?>]" class="Estilo2" size="80"  disabled value="<? echo htmlspecialchars($row3["doc_especificacion"]) ?>" ></td>
													...
													<td class="Estilo1" colspan=1>
														<?php if ($v_oc_sp_id == "" || $v_oc_sp_id == 0): ?>

															<?php 
															if (!isset($_GET["oc_wms"])) {
																$oc_wms="";
															} 
															?>
															<a href="bode_inv_despachos_br.php?ori=<? echo $ori ?>&id=<? echo $id ?>&doc_id=<? echo $row3["doc_id"] ?>&doc_id2=<? echo $row3["doc_origen_id"] ?>&cantidad=<? echo $row3["doc_cantidad"] ?>&oc_wms=<?php echo $oc_wms ?>&origen_id=<?php echo $row3["doc_origen_id"] ?>" class="link" onClick="return confirm('Seguro que desea Cambiar Estado ?')" ><i class="fa  fa-times"></i></a>
														<?php else: ?>
															<a href="#" onclick="abrirVentana(<?php echo $row3["doc_sp_rel_doc_id"] ?>,<?php echo $v_oc_sp_id ?>)"><i class="fa fa-eye link"></i></a>
														<?php endif ?>
													</td>
												</tr>
												<?
...
<form action="bode_guia_despacho_gr.php" method="POST" onSubmit="return check()" enctype="multipart/form-data" name="form1" id="form1">
	...
	// $folio = intval($folio["Folio"] + 1);
			...
			<?php else: ?>
				...
				<?php endif ?>
			<?php 
			if($totalInventariables2 > 0){
				$res66 = mysql_query("SELECT * FROM bode_detoc a INNER JOIN bode_orcom b ON b.oc_id = a.doc_oc_id INNER JOIN bode_detingreso c ON c.ding_id = a.doc_origen_id WHERE b.oc_id = ".$id." AND a.doc_estado <> 'ELIMINADO' and c.ding_clasificacion = 1");
				$contador3 = 1;

			 ?>
				<?php while($row66=mysql_fetch_array($res66)) { ?>

				<!-- 

onblur="validaRango(<?php echo $row66["doc_cantidad"] ?>,<?php echo $contador3 ?>)" 
				 -->
				<tr>
					<td class="Estilo1">RANGO N° <?php echo $contador3 ?></td>
					<td class="Estilo1">DESDE : <input type="text" name="desde[<?php echo $contador3 ?>]" id="desde_<?php echo $contador3 ?>" class="Estilo1" onkeypress='return event.charCode >= 48 && event.charCode <= 57'> - HASTA <input type="text" name="hasta[<?php echo $contador3 ?>]" id="hasta_<?php echo $contador3 ?>" class="Estilo1" onkeypress='return event.charCode >= 48 && event.charCode <= 57' > <?php echo $row66["doc_cantidad"] ?></td>
				</tr>
				<?php $contador3++;} ?>
			...
			<tr>
				...
				<td class="Estilo1" colspan="2">
					<?php if ($tipo=="Inventariable"): ?>
						<?php
						echo "<textarea name=\"obs\" id=\"obs\" style=\"margin: 0px; padding: 0px; width: 725px; height: 247px;\">";
						...
						// 	}
						// }
						echo $v_oc_observaciones;
						echo "\nMERCADERÍA EN TRASLADO PROPIEDAD DE JUNJI. OPERACIÓN NO CONSTITUYE VENTA</textarea>";
						?>
					<?php else: ?>
						<textarea name="obs" id="obs" style="margin: 0px; padding: 0px; width: 725px; height: 247px;"><?php echo $v_oc_observaciones."\n" ?>MERCADERÍA EN TRASLADO PROPIEDAD DE JUNJI. OPERACIÓN NO CONSTITUYE VENTA</textarea
						<?php endif ?>
					</td>
				</tr>

				<?php if ($v_oc_mas_id <> 0 || $v_oc_sp_id <> 0 || $_GET["oc_wms"] <> ""): ?>
				...
				<?php if ($contador > 1): ?>
					...
					<tr>
						<td></td>
						<td><button type="submit" id="btnCerrar">Cerrar guía</button></td>
					</tr>
				<?php endif ?>
			...
			<input type="hidden" name="ori" value="<? echo $ori ?>"  >
			<!-- <input type="te" name="totalElementos" id="totalElementos" value="<?php echo ($v_oc_sp_id > 0) ? $totalInventariables : mysql_num_rows($res3) ?>"> -->
			<input type="hidden" name="totalElementos" id="totalElementos" value="<?php echo mysql_num_rows($res3) ?>">
			<!-- NUEVO !-->
			<input type="hidden" name="tipo" id="tipo" value="<?php echo $tipo ?>">
			<input type="hidden" name="inv_doc_id" value="<?php echo $doc_id["ding_id"] ?>">
			...
			<input type="hidden" name="region_destino" value="<?php echo ($v_oc_sp_id == "" || $v_oc_sp_id == 0) ? $v_oc_region_destino : $row7["sp_region"] ?>">
			<input type="hidden" name="origen_despacho" id="origen_despacho" value="1">
		</form>
	<!-- <table border="0" width="100%">
		<tr>
...
<script type="text/javascript">
	...
	function validaRango(cantidad,id)
	{
		var inicio = parseInt($("#desde_"+id).val());
		var fin    = parseInt($("#hasta_"+id).val());
		var totalCodigos = (fin - inicio) + 1;

		if(inicio == "" || isNaN(inicio))
		{
			alert("El rango de inicio está vacio");
			$("#desde_"+id).focus();
			return false;
		}
		if(fin == "" || isNaN(fin))
		{
			alert("El rango final está vacio");
			$("#hasta_"+id).focus();
			return false;
		}
		...
		if(totalCodigos != cantidad)
		{
			alert("RANGO DE CODIGOS NO COINCIDE CON CANTIDAD DE PRODUCTOS INGRESADOS");
			$("#desde_"+id).val("");
			$("#hasta_"+id).val("");
			$("#desde_"+id).focus();
			return false;
	...
	{
		var oc_tipo_guia = "<?php echo $v_oc_tipoguia ?>";
		$("#btnCerrar").hide();
		var data = ({cmd:"validaFolio", folio:input,tipo_guia : oc_tipo_guia});
		$.ajax({
			type:"POST",
	...
	function check(){
		...
		var inventariables2 = 0;
		// console.log(totalInventariables);
				...
				if($("#desde_"+i).val() != '' && $("#hasta_"+i).val() != '')
				{
					...
					if(parseInt($("#desde_"+i).val()) > parseInt($("#hasta_"+i).val()))
					{
						...
						if(confirm('PARA EL RANGO N° '+i+' SE ACTUALIZARÁN '+totalRango+' CODIGOS DE INVENTARIO, ¿ES CORRECTA LA INFORMACIÓN ?'))
						{
							...
							alert("POR FAVOR VERIFIQUE LOS CODIGOS DE INVENTARIO EN EL RANGO N° "+i);
							return false;
						}
					}			
				}
		...
		if(totalInventariables > 0){
			if(inventariables2 == 0 && $("#separados").val() == "")
			{
				alert("Debe ingresar los codigos de inventario!");
				return false;
			}
		}
...
/*
		if(totalInventariables > 0)
		{
			for(x=1;x<=totalInventariables;x++)
			{
			}
		}
		*/
		...
		var v_oc_mas_id = '<?php echo $v_oc_mas_id ?>';
		var v_oc_sp_id = '<?php echo $v_oc_sp_id ?>';
		if( (v_oc_mas_id == '0' || v_oc_mas_id == "") && (v_oc_sp_id == '0' || v_oc_sp_id == ""))
		{
	...
	if(totalElementos > 30)
	{
		alert("SE HA LLEGADO AL LIMITE DE PRODUCTOS (30)");
		return false;
	}
...
function valida(){
	...
	if(numberOfChecked == 0){
		...
		return false;
	}else if(totalElementos > 30){
		alert("SE HA SUPERADO EL LIMITE DE 30 ELEMENTOS");
		return false;
	}else{
		blockUI();
		return true;
...
function abrirVentana(doc_id,sp_id)
{
	window.open("bode_solicitud_popup_detalle2.php?id="+doc_id+"&sp_id="+sp_id,"miwin","channelmode=0,directories=0,fullscreen=0,height=500,location=0,menubar=0,resizable=0,scrollbars=1,status=0,titlebar=0,toolbar=0,width=1000")
}
</script>
