		...
		<div class="row-fluid">
			...
			<div class="row-fluid">
			   ...
			   <div class="box span12">
				  ...
				  ?>
			      <a href="ver_oc.php?id_region=17" class="label <? echo $cod_color ?>">Todas</a>&nbsp;&nbsp;
			      <?
				   ...
				   ?>
                     <a href="ver_oc.php?id_region=<? echo $v_region_id ?>" class="label <? echo $cod_color ?>"><? echo $v_region_glosa ?></a>&nbsp;&nbsp;
					...
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
						    <?
							    if ($id_region==17){
							          $consulta="select * from bode_orcom where oc_tipo = 0 AND YEAR(oc_fecha_recep) >= 2016 AND oc_estado = 1";
							    } else if($id_region == 16 or $id_region == 13){
								      $consulta="select * from bode_orcom where oc_tipo = 0 and (oc_region=13 or oc_region = 16) AND YEAR(oc_fecha_recep) >= 2016 AND oc_estado = 1";
                                }else{
                                	$consulta="select * from bode_orcom where oc_tipo = 0 and oc_region='$id_region' AND YEAR(oc_fecha_recep) >= 2016 AND oc_estado = 1";
                                }
								$res = mysql_query($consulta);
								while ($arr=mysql_fetch_array($res)){
								    $v_oc_id	        = $arr['oc_id'];
							...
							<tr>
								...
								<td><? echo $v_oc_nombre_oc ?></td>
								<td><? echo $v_oc_prog ?></td>
								<td><? echo $v_oc_fecha ?></td>
								<td><? echo $v_oc_fecha_recep?></td>
								<td>$<? echo number_format($v_oc_monto,0,".",".") ?></td>
								<td><? echo $v_oc_proveenomb ?></td>
								<td><? echo $v_oc_estado ?></td>
								<td class="center">
									<a class="btn btn-success" href="ver_oc_det.php?id_oc=<? echo $v_oc_id ?>">
