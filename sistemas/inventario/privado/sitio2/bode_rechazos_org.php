	...
	<div style="background-color:#E0F8E0; position:absolute; top:120px; left:00px; width:100%" id="div1">
		...
		extract($_POST);
			...
			if(isset($enviar) && $enviar == "BUSCAR")
			{
				...
				$sql = "SELECT * FROM bode_ingreso a INNER JOIN bode_orcom b ON a.ing_oc_id = b.oc_id WHERE a.ing_guianumerorrchzo <> 0  $where1 $where2 $where3 $where4 ORDER BY ing_id DESC";
			}else{
				$sql = "SELECT * FROM bode_ingreso a INNER JOIN bode_orcom b ON a.ing_oc_id = b.oc_id WHERE a.ing_guianumerorrchzo <> 0 ORDER BY ing_id DESC";
			}
		...
		if(isset($ing_id) && intval($ing_id))
		{
			...
			echo "<script>window.location.href='bode_rechazos.php?cod=40';</script>";
		//echo $rechazado;
		}
