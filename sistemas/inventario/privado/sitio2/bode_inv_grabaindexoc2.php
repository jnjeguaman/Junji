...
$mensaje = "";
		if ($archivo1 != "") {
			...
			{
				$archivo1 = "distribucion_".$regionsession."_".strtoupper($oc).".".$extension;
				$ruta1="archivos/docdist/".date("Y")."/".$regionsession."/";
...
//FIN ARCHIVO DE DISTRIBUCIÓN
		if(strlen($mensaje) > 0)
		{
			...
			$sql = "INSERT INTO bode_orcom (oc_id2, oc_region,   oc_nombre_oc, oc_prog,        oc_fecha,    oc_fecha_recep,     oc_usu, oc_pro_id, oc_proveerut,   oc_proveedig, oc_proveenomb, oc_cantidad, oc_monto, oc_descuento, oc_numerooc, oc_grupo, oc_estado,oc_sc,oc_envio_fecha,oc_rutatc,oc_archivotc )
						VALUES ( '$oc', '$regionsession', '".mysql_real_escape_string($nombreoc)."',    '$programa', '$fecha2', '$fechamia', '$usuario', '0',             '$proveedor', '$proveedor2',  '$proveedornomb'   ,'$cantidad',  '$total', '$descuento', '$oc',  '$grupo',1,'$sc','$f_oc','$ruta1','$archivo1');";
...
$oc_conversion = $total * $tipo_cambio;
	$sql = "INSERT INTO bode_orcom (oc_id2, oc_region,   oc_nombre_oc, oc_prog,        oc_fecha,    oc_fecha_recep,     oc_usu, oc_pro_id, oc_proveerut,   oc_proveedig, oc_proveenomb, oc_cantidad, oc_monto, oc_descuento, oc_numerooc, oc_grupo, oc_estado,oc_sc,oc_envio_fecha,oc_conversion)
						VALUES ( '$oc', '$regionsession', '".mysql_real_escape_string($nombreoc)."',    '$programa', '$fecha2', '$fechamia', '$usuario', '0',             '$proveedor', '$proveedor2',  '$proveedornomb'   ,'$cantidad',  '$total', '$descuento', '$oc',  '$grupo',1,'$sc','$f_oc','$oc_conversion');";
// Agregar el Proveedor a la BD
$buscar = "SELECT COUNT(proveedor_id) as Total FROM acti_proveedor WHERE proveedor_rut = '".$proveedor."'";
$buscar = mysql_query($buscar);
$buscar = mysql_fetch_array($buscar);
